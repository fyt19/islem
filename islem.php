...
// header renk ayarı
$sql = "SELECT header_color FROM ayar LIMIT 1";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
	// Veriyi bir satır olarak çek
	$row = $result->fetch_assoc();
	$header_color = $row['header_color']; // header_color alanı
} else {
	// Eğer veri bulunamazsa bir varsayılan renk kullan
	$header_color = "#ffd54c"; // Varsayılan renk
}
...
if (isset($_POST['markaekle'])) {
	...
	$marka_seourl = seo($_POST['marka_ad']);

	$kaydet = $db->prepare("INSERT INTO marka SET
		marka_ad=:marka_ad,
		markafoto_yolu=:markafoto_yolu		
	");
	$insert = $kaydet->execute(array(
		'marka_ad' => $_POST['marka_ad'],
		'markafoto_yolu' => $_POST['markafoto_yolu']
	));
...
if (isset($_POST['markaduzenle'])) {
	...
	if ($update) {
		Header("Location:../production/marka-duzenle.php?durum=ok&marka_id=$marka_id");
	} else {
		Header("Location:../production/marka-duzenle.php?durum=no&marka_id=$marka_id");
	}
}
...
if (isset($_POST['faviconguncelle'])) {
	...
	if ($update) {
		// Eski faviconu silme
		$eski_yol = $_POST['eski_favicon_yol'];
